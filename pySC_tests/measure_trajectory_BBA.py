...
from pySC.apps.bba import analyze_trajectory_bba_data, get_trajectory_bba_analysis_data
...
with open('tbba_config.json','r') as fp:
...
#    logger.debug(f"Got code: {code.name}")
    if code == BBACode.HORIZONTAL_DONE:
        offset_h, offset_h_err = analyze_trajectory_bba_data(measurement_object.H_data, n_downstream=n_downstream)
        logger.info(f"BBA offset H = {offset_h*1e6:.1f} +- {offset_h_err*1e6:.1f} μm.")
    if code == BBACode.VERTICAL_DONE:
        offset_v, offset_v_err = analyze_trajectory_bba_data(measurement_object.V_data, n_downstream=n_downstream)
...
def plot_data(data):
    ...
    bpm_pos_h, orbits_h, slopes_h, center_h, final_mask_h, off_h = get_trajectory_bba_analysis_data(data, n_downstream=n_downstream)
